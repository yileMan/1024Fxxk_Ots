...
覆盖数据时，管理员摘要显示“未提供”。漏洞事实和候选区域保持只读；OTS-12 只在服务端明确返回可见当前
评估 ID 时增加评估入口，不根据漏洞或产品参数猜测 ID。

OTS-12 新增 `/system/assessments/:assessmentId` 单页评估详情。负责人可从待办或漏洞详情入口读取来源事实、
候选依据、退回/复评原因和当前产品结论，并通过“保存草稿”明确保存；保存不会提交审核。页面支持分析摘要、
触发条件、涉及功能或接口、适用性及依据、产品影响、现有控制、处置方式及说明和证据说明，所有外部文本均
按纯文本展示。服务端字段校验会定位到具体输入；并发冲突保留本地内容并提供刷新动作。管理员、审核人、
非负责人、历史修订和非草稿状态均按服务端 `editable` 判定只读，不显示可用保存操作。

OTS-15 在同一评估详情页增加按需加载的修订历史、当前/历史标记和同链字段差异。历史修订保持只读；审核退回
后页面采用服务端返回的新当前 ID，并置顶显示退回意见。当前负责人可编辑并重新提交 `returned/reassess`，也可
从当前已完成修订输入原因创建人工复评修订。冲突不会自动覆盖本地输入；服务端返回当前修订 ID 时，“刷新服务器
数据”会明确切换到该当前修订。所有原因、意见和差异仍按不可信纯文本渲染。

来源变化自动复评、跨产品参考和全局追溯不在 OTS-15 范围，分别留给 OTS-16、OTS-17 和 OTS-19。
