<template>
  <main class="assessment-page">
    ...
    <template v-else-if="detail">
      ...
      <section v-if="detail.reassessment" class="reassessment-card" data-reassessment aria-labelledby="reassessment-title">
        <header>
          <div><small>CHANGE SIGNAL / 自动复评变化</small><h2 id="reassessment-title">评估依据已变化</h2></div>
          <div class="change-types"><span v-for="type in detail.reassessment.change_types" :key="type">{{ changeTypeLabels[type] ?? type }}</span></div>
        </header>
        <p v-if="detail.status==='submitted'" class="submitted-change-warning" data-submitted-change-warning role="alert">该变化发生在提交后。审核前请重新核对来源依据与产品结论。</p>
        <dl class="basis-changes">
          <div v-for="change in detail.reassessment.changes" :key="change.field">
            <dt>{{ change.field }}</dt>
            <dd><span>{{ displayDiffValue(change.before) }}</span><i aria-hidden="true">→</i><strong>{{ displayDiffValue(change.after) }}</strong></dd>
          </div>
        </dl>
        <footer><span>触发时间 {{ new Date(detail.reassessment.triggered_at).toLocaleString() }}</span><span>基线 {{ detail.reassessment.basis_sha256.slice(0,12) }}</span><strong v-if="detail.reassessment.truncated_count">另有 {{ detail.reassessment.truncated_count }} 项变化未展示</strong></footer>
      </section>
...
.reassessment-card{margin-top:12px;border:1px solid var(--line-strong);border-left:7px solid #d97706;padding:22px;background:#fffaf0}.reassessment-card>header{display:flex;align-items:flex-start;justify-content:space-between;gap:20px}.reassessment-card small{color:#9a4b05;font-size:9px;font-weight:900;letter-spacing:.14em}.reassessment-card h2{margin:5px 0 0;font:750 26px var(--font-display)}.change-types{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:6px}.change-types span{border:1px solid #b65d09;padding:4px 7px;color:#753805;background:#fff;font-size:10px;font-weight:900}.submitted-change-warning{padding:12px 14px;border-left:5px solid var(--brand-red);color:var(--brand-red-deep);background:#fff;font-weight:800}.basis-changes{display:grid;gap:8px}.basis-changes>div{display:grid;grid-template-columns:minmax(150px,.5fr) 1.5fr;gap:12px;padding:11px;background:#fff}.basis-changes dt{font:700 11px/1.5 Consolas,monospace}.basis-changes dd{display:grid;grid-template-columns:1fr auto 1fr;gap:10px;margin:0;overflow-wrap:anywhere;font-size:12px}.basis-changes dd span{color:var(--text-muted)}.basis-changes dd i{font-style:normal}.basis-changes dd strong{color:var(--ink)}.reassessment-card footer{display:flex;flex-wrap:wrap;gap:16px;margin-top:16px;color:var(--text-muted);font-size:10px}.reassessment-card footer strong{color:#8b4307}@media(max-width:780px){.reassessment-card>header{display:grid}.change-types{justify-content:flex-start}.basis-changes>div{grid-template-columns:1fr}.basis-changes dd{grid-template-columns:1fr}.basis-changes dd i{transform:rotate(90deg);justify-self:start}}
