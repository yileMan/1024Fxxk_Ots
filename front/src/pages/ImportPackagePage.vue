  ...
  <main class="package-page">
    ...
    <template v-if="result">
      ...
      <section v-if="matching" class="matching-ledger" aria-label="内部 OTS 候选匹配">
        ...
        <section class="task-ledger" aria-label="产品评估任务生成">
          <div class="task-heading">
            <div><small>PRODUCT ASSESSMENT TASKS</small><h3>产品评估任务</h3></div>
            <strong :class="{ pending: !taskGeneration || taskGeneration.status !== 'succeeded' }">{{ taskGeneration?.status === 'succeeded' ? '任务已生成' : '产品任务待生成' }}</strong>
          </div>
          <template v-if="taskGeneration">
            <div class="task-metrics">
              <article><small>新任务</small><strong>{{ taskGeneration.task_inserted_count }}</strong></article>
              <article><small>待复评</small><strong>{{ taskGeneration.task_reassess_count }}</strong></article>
              <article><small>已更新</small><strong>{{ taskGeneration.task_updated_count }}</strong></article>
              <article><small>未变化</small><strong>{{ taskGeneration.task_unchanged_count }}</strong></article>
              <article><small>已跳过</small><strong>{{ taskGeneration.task_skipped_count }}</strong></article>
              <article><small>失败</small><strong>{{ taskGeneration.task_failed_count }}</strong></article>
            </div>
            <article v-for="task in taskGeneration.task_samples" :key="`${task.vulnerability_id}-${task.product_ots_id ?? task.reason}-${task.action}`" class="task-card">
              <div><strong class="mono">{{ task.cve_id }}</strong><span>{{ task.action }}</span></div>
              <h4>{{ task.product_name ? `${task.product_name} ${task.version_no ?? ''}` : '暂无有效产品关联' }}</h4>
              <p>{{ task.reason ?? `负责人 #${task.owner_id} · 产品 OTS #${task.product_ots_id}` }}</p>
            </article>
            <div v-if="Object.keys(taskGeneration.skip_reason_counts).length" class="skip-reasons">
              <p v-for="(count, reason) in taskGeneration.skip_reason_counts" :key="reason"><strong class="mono">{{ reason }}</strong><span>{{ count }}</span></p>
            </div>
            <p v-if="taskGeneration.truncated_task_count" class="truncated">另有 {{ taskGeneration.truncated_task_count }} 条任务差异未在页面展示。</p>
          </template>
          <p v-else class="legacy-task-note">该结果来自 OTS-08，尚未生成产品任务；重新执行将按当前产品范围幂等补齐。</p>
          <p class="task-warning">候选不等于产品受影响，所有任务均为待产品独立评估事项。</p>
        </section>
        <button v-if="matching.status !== 'succeeded' || taskGeneration?.status !== 'succeeded'" data-action="execute-matches" class="primary" :disabled="matchingBusy" @click="executeMatches">{{ matchingBusy ? '正在提交…' : '执行内部匹配与任务生成' }}</button>
      </section>
      <p v-if="matchingError" class="feedback error" role="alert">{{ matchingError }}，候选与产品任务均已回滚，漏洞事实导入结果不受影响，可稍后重试。</p>
...
const taskGeneration = computed(() => matching.value?.task_generation ?? null)
...
async function executeMatches(): Promise<void> {
  if (!result.value || matchingBusy.value) return
  if (!window.confirm(`确认对批次 ${result.value.batch_no} 执行内部 OTS 候选匹配并生成产品评估任务？候选不等于产品受影响。`)) return
  matchingBusy.value = true
  matchingError.value = ''
  try {
...
.matching-ledger{margin-top:12px;border:1px solid var(--line-strong);background:#fff}.matching-metrics{display:grid;grid-template-columns:repeat(5,1fr);gap:1px;background:var(--line)}.matching-metrics article{padding:18px;background:#fff}.matching-metrics small,.matching-metrics strong{display:block}.matching-metrics strong{margin-top:8px;font:750 26px var(--font-display)}.match-card{padding:18px 24px;border-top:1px solid var(--line)}.match-card>div{display:flex;gap:10px}.match-card span{padding:3px 7px;background:var(--paper-warm);font-size:10px}.match-card h3{margin:10px 0 5px}.match-card p,.unmatched-list,.candidate-warning{color:var(--text-muted)}.unmatched-list,.candidate-warning{margin:0;padding:14px 24px;border-top:1px solid var(--line)}.candidate-warning{color:var(--brand-red-deep);font-weight:800}.matching-ledger>.primary{margin:0 24px 24px}.task-ledger{margin:0 24px 20px;border:1px solid var(--line-strong);background:var(--paper-warm)}.task-heading{display:flex;align-items:center;justify-content:space-between;padding:18px 20px;color:#fff;background:var(--ink)}.task-heading small{color:rgba(255,255,255,.58);font-size:9px;font-weight:900;letter-spacing:.14em}.task-heading h3{margin:5px 0 0;font:750 21px var(--font-display)}.task-heading>strong{padding:6px 9px;border:1px solid rgba(255,255,255,.28);font-size:10px}.task-heading>strong.pending{color:#271d10;background:#f2c66d;border-color:#f2c66d}.task-metrics{display:grid;grid-template-columns:repeat(6,1fr);gap:1px;background:var(--line)}.task-metrics article{padding:15px;background:#fff}.task-metrics small,.task-metrics strong{display:block}.task-metrics small{color:var(--text-muted);font-size:9px}.task-metrics strong{margin-top:7px;font:750 23px var(--font-display)}.task-card{padding:15px 20px;border-top:1px solid var(--line);background:#fff}.task-card>div{display:flex;gap:9px}.task-card span{padding:3px 7px;background:var(--ink);color:#fff;font-size:9px}.task-card h4{margin:8px 0 4px}.task-card p,.legacy-task-note,.task-warning{margin:0;color:var(--text-muted)}.skip-reasons{display:flex;flex-wrap:wrap;gap:8px;padding:13px 20px;border-top:1px solid var(--line)}.skip-reasons p{display:flex;gap:8px;margin:0;padding:6px 8px;background:#fff;font-size:10px}.legacy-task-note,.task-warning{padding:15px 20px}.task-warning{border-top:1px solid var(--line);color:var(--brand-red-deep);font-weight:800}@media(max-width:900px){.matching-metrics{grid-template-columns:repeat(2,1fr)}.task-metrics{grid-template-columns:repeat(3,1fr)}}@media(max-width:600px){.task-ledger{margin-inline:12px}.task-metrics{grid-template-columns:repeat(2,1fr)}}
</style>
