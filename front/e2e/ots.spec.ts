...
test('管理员可从一级菜单和工作台进入 OTS 主数据', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'OTS' })).toBeVisible()
  await expect(page.getByText('OTS 主数据')).toBeVisible()
  await page.getByRole('link', { name: 'OTS' }).click()
...
test('非管理员不能进入 OTS 管理', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'OTS' })).toHaveCount(0)
})
