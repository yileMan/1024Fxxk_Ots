...
| 本版调整 | 删除 OTS 情报分析员和独立情报分析环节；导入成功后直接生成产品评估待办并站内通知产品负责人；保留其他产品已审核评估结论的参考能力 |

## 1. 方案结论

V1 采用“外网采集工具 + 离线 ZIP/CSV 数据包 + 内网管理平台”的形式。

- 管理平台先根据实际在用 OTS 导出采集范围，外网采集工具据此访问 NVD、CISA KEV 和 endoflife.date，并在外网侧过滤无关数据；
...
- 读取管理平台导出的采集范围 CSV；
- 按范围内 OTS 和 NVD `lastModified` 时间进行增量采集；
- 同步 CISA KEV 官方目录后只保留与范围内候选 CVE 相关的记录；
- 只为范围内 OTS 获取 endoflife.date 生命周期候选信息；
- 对数据进行基础规范化、关联和校验；
- 在外网侧过滤与采集范围无关的数据，生成可导入内网的版本化 ZIP 数据包；
- 保存采集游标和采集日志。

采集工具不维护产品评估结论，不访问内网数据库，不直接向管理平台推送。
...
| 后端 | Python FastAPI | 与采集工具语言统一，便于处理 CSV、ZIP 和评分数据 |
| 数据访问 | SQLAlchemy + Alembic | 提供关系映射和数据库版本升级 |
...
采集工具优先使用数据源提供的 OTS、版本或时间过滤能力。数据源不能精确过滤时，允许在外网侧临时读取较大集合，但必须在生成回传包之前与 `collector_scope.csv` 做匹配和过滤。只有与至少一个范围内 OTS 形成候选匹配的 CVE 才能进入数据包。

#### NVD

- 每个 OTS 使用自己的 `last_covered_time` 形成增量窗口，不只按 CVE 发布时间；
- 保存 CVE ID、状态、发布时间、最后修改时间、英文描述、CWE、CPE 配置、参考链接及来源提供的 CVSS；
- 采集工具处理分页、限流、超时和重试；
- 相邻增量窗口保留少量重叠，由内容摘要和唯一键去重，避免边界遗漏。
- 名称、版本和官方网站只能生成候选匹配，不能自动形成产品受影响结论；匹配方式、依据和可选置信度写入 `matches.csv`，导入后供各产品负责人评估时参考。

#### CISA KEV

- 每次采集可读取官方完整目录，但只按 CVE ID 保留已经与范围内 OTS 形成候选匹配的记录；
- 保存加入日期、要求处置日期、勒索活动标记和官方处置说明；
- KEV 新增或内容变化在导入后触发相关记录待复核。

#### EOL

- 根据 OTS 名称和版本查询可用的生命周期候选信息；
- 导入结果只作为候选信息；
- 审核人确认后才更新 OTS 的 `is_eol` 当前结论；
- 有供应商或项目官方来源时，人工补充并优先采用官方来源；
- 无可靠信息时不更新当前结论，也不推断为“未 EOL”。

采集工具为每个范围 OTS 输出 `succeeded`、`failed` 或 `not_run`，并仅为 `succeeded` 输出新的覆盖截止时间。成功完成但没有新增匹配只表示该覆盖窗口内没有新的匹配候选，不表示该 OTS 没有漏洞。
...
| manifest.csv | 包格式版本、批次号、生成时间、采集工具版本、范围导出 ID、范围文件摘要、各文件摘要和逐 OTS 采集结果 |
| vulnerabilities.csv | CVE 基础事实及来源时间 |
| affected_ranges.csv | CVE、CPE 和受影响版本范围 |
| cvss_scores.csv | 多来源的 CVSS v3.1/v4.0 分数、向量和严重度 |
...
1. 外网采集工具生成 OTS/CVE 候选匹配，提供匹配方式、匹配依据和可选置信度；
...
每个 CVE 当前记录分别保存一组 CVSS v3.1 和一组 CVSS v4.0 来源评分；每组包括基础分、严重度、完整向量和评分来源。导入时按既定来源优先级选择当前采用值，并通过来源修改时间和内容哈希识别变化。

产品选择的主要 CVSS 版本只影响默认展示和产品环境评分；某版本无来源数据时显示“来源未提供”，不在不同 CVSS 版本之间换算。

### 6.2 产品环境评分

- CVSS v3.1 和 v4.0 使用各自独立的指标表单和计算器；
- 前端提交指标，后端重新计算分数和向量；
- 每次提交保存指标、向量、分数、计算器版本和评估依据；
- 来源没有所选版本时显示“来源未提供”，不从另一版本换算；
...
1. 产品基本信息；
2. 产品版本、主要 CVSS 版本、指定产品负责人和指定审核人；
3. OTS 清单。
...
- NVD 分页、限流、重试和 `lastModified` 增量；
- CISA KEV 全量同步和关联；
- endoflife.date 无数据、字段缺失和来源冲突；
- 采集范围导出到数据包导入的完整流程；
...
- 按 OTS 及增量时间采集 NVD、CISA KEV、endoflife.date，并在外网侧过滤；
