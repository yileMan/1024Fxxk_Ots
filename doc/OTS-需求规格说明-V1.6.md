...
| 本版调整 | 删除 OTS 情报分析员角色及独立情报分析环节；外部数据成功导入后直接为相关产品版本生成评估待办并通知产品负责人；保留其他产品已审核评估结论的参考能力 |
...
2. 管理平台根据实际在用 OTS 生成最小采集范围，在外网按该范围采集 NVD、CISA KEV 和 EOL 信息，并只将相关数据通过离线包导入内网；
3. 外部数据成功导入后，直接为使用相关 OTS 的不同产品版本生成独立评估任务，由产品负责人开展适用性、影响和处置评估；
4. 保存来源提供的 CVSS v3.1、CVSS v4.0 信息，并支持产品环境评分；
...
- NVD、CISA KEV、endoflife.date 数据采集工具；
- 版本化 ZIP/CSV 数据包生成、校验、预览和导入；
- CVE、CWE、受影响版本、参考链接、CVSS v3.1/v4.0、KEV 和 EOL 信息管理；
...
| CVE/OTS 候选关联 | 外网采集工具根据 CPE、名称、版本、厂商公告等形成并随数据包导入的候选关系，保存匹配方式和匹配依据；不等同于具体产品受影响结论 |
...
管理平台先从启用产品、启用产品版本及产品 OTS 关联中生成去重后的采集范围，外网采集工具仅回传与该范围相关的漏洞候选。自动匹配只生成 OTS/CVE 候选关系，不自动得出产品“受影响”结论。数据包成功导入后，系统立即为每个相关有效产品 OTS 创建或更新独立产品评估任务，并通过站内工作台通知该产品版本当前负责人。产品负责人评估时可以查看相同 OTS/CVE 在其他产品中已审核通过的评估结论，但必须独立确认本产品结论。
...
| FR-MASTER-003 | 产品版本至少包含版本号、说明、主要 CVSS 版本、当前产品负责人、当前审核人和状态。 |
...
| FR-MASTER-007 | 产品版本应选择主要 CVSS 评估版本：CVSS v3.1 或 CVSS v4.0。 |
...
| FR-EXCH-003 | 采集工具应从 NVD 获取 CVE、发布时间、最后修改时间、描述、CWE、受影响范围、参考链接和来源 CVSS。 |
| FR-EXCH-004 | 采集工具应从 CISA KEV 官方目录获取 KEV 信息。 |
| FR-EXCH-005 | 采集工具应从 endoflife.date 获取 EOL 候选信息。 |
| FR-EXCH-006 | 采集工具应读取采集范围，并优先按 OTS、版本及增量时间调用数据源；数据源无法精确按 OTS 查询时，可在外网侧获取较大集合，但必须在生成数据包前完成本地过滤。 |
...
| FR-EXCH-013 | 采集工具应输出一个版本化 ZIP 数据包，包内包含多张关联 CSV、原采集范围导出 ID、范围文件摘要和批次说明。 |
...
| FR-VULN-002 | 同一 CVE 应保存来源提供的 CVSS v3.1/v4.0 分数、严重度、向量和评分来源。 |
| FR-VULN-003 | 来源未提供某版本评分时显示“来源未提供”，不得自动换算。 |
...
| 编号 | 需求 |
| --- | --- |
| FR-MATCH-001 | 外网采集工具应为范围内漏洞生成 OTS/CVE 候选关联，并随数据包提供匹配方式、匹配依据和可选置信度。 |
...
3. 产品选择的 CVSS 版本只影响默认展示和产品环境评分，不裁剪来源数据；
...
- 产品评估和审核使用同一详情页；
- 产品版本表单应指定主要 CVSS 版本、产品负责人和审核人，OTS 表单只维护名称、版本、官方网站和是否 EOL；
...
3. 管理平台能够导出采集范围，外网采集工具能够生成版本化 ZIP/CSV 数据包；
...
9. 平台能够保存并区分 CVE、CWE、受影响范围、参考链接、CVSS v3.1/v4.0、KEV 和 EOL；
