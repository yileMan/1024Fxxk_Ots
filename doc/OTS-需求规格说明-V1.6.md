...
| 本版调整 | NVD 数据改为先导入原始 CVE 事实和受影响软件/版本范围，再由内网平台匹配 OTS 并生成评估待办；KEV/EOL 作为后续可选输入 |
...
2. 管理平台接收外部数据服务提供的 NVD 原始漏洞事实和受影响软件/版本范围；实际在用 OTS 采集范围可作为外部工具的可选辅助输入；
3. 漏洞事实导入后，平台内部匹配 OTS，并为使用候选 OTS 的不同产品版本生成独立评估任务，由产品负责人开展适用性、影响和处置评估；
...
| 数据采集 | 由项目边界外的独立外部数据服务完成；本项目接收符合契约的 NVD 原始事实包，不实现联网采集 |
| 数据交换 | 通过一个 ZIP 数据包离线交换；NVD `1.0` 包固定为 `manifest.csv` 和 `nvd_cves.csv` |
| 采集范围 | 管理平台可按实际在用 OTS 导出去重范围供外部工具优化采集；该范围不是 NVD `1.0` 回传包的组成部分或导入前置条件 |
...
| 通知 | 内部候选匹配并生成评估任务后，通过站内工作台通知产品负责人；V1 不实现邮件通知 |
...
- 平台内部 OTS/CVE 候选匹配、产品适用性评估、环境评分、影响分析和处置建议；
- 审核通过、退回、重新提交和修订留痕；
...
| CVE/OTS 候选关联 | 平台依据已导入的受影响软件/版本范围与内部 OTS 主数据计算的候选关系，保存匹配方式和匹配依据；不等同于具体产品受影响结论 |
...
| 产品负责人 | 完成所负责产品版本的漏洞评估 | 接收内部匹配后生成的站内待办，参考来源事实、候选匹配依据和其他产品已审核结论，填写适用性、环境评分、影响、措施和依据，提交审核 |
...
```mermaid
flowchart TD
    A["外部数据服务生成 NVD 事实包"] --> B["内网校验并导入漏洞事实"]
    B --> C["平台内部匹配 OTS"]
    C --> D["生成待办并通知产品负责人"]
    D --> F["产品负责人评估"]
    F --> G["审核人审核"]
...
外部数据服务按来源时间窗口提供 NVD 原始 CVE 事实、公开软件身份和受影响版本范围，不接收或回传内网 OTS ID。本项目不实现外部服务的数据源访问。管理员确认导入漏洞事实后，平台内部将受影响范围与 `ots_component` 匹配；自动匹配只生成 OTS/CVE 候选关系，不自动得出产品“受影响”结论。随后系统为每个相关有效产品 OTS 创建或更新独立产品评估任务，并通过站内工作台通知该产品版本当前负责人。采集范围导出仍可作为外网工具的可选辅助，但不约束 NVD `1.0` 导入包。产品负责人评估时可以查看相同 OTS/CVE 在其他产品中已审核通过的评估结论，但必须独立确认本产品结论。
...
| FR-EXCH-003 | NVD 数据包每行应表示一个 CVE，并提供 CVE ID、来源标识、来源状态、发布时间、最后修改时间、描述、受影响软件/版本范围、全部 CWE、参考链接、来源 CVSS 和原始 configuration；本项目校验、预览并导入这些事实。 |
| FR-EXCH-004 | 外部数据服务输入的数据包可提供 KEV 信息；本项目只校验、预览和导入这些字段。 |
| FR-EXCH-005 | 外部数据服务输入的数据包可提供 EOL 候选信息；本项目只校验、预览和导入这些字段。 |
| FR-EXCH-006 | 外部数据服务可读取采集范围以缩小采集量，但 NVD `1.0` 包不得依赖或携带内网 OTS ID；本项目不实现数据源调用。 |
| FR-EXCH-007 | NVD `1.0` 包可包含来源时间窗口内的完整 CVE 事实；无 configuration、Rejected 或暂时匹配不到内部 OTS 的 CVE仍允许导入。 |
| FR-EXCH-008 | 导入前应展示新增、更新、重复、冲突和错误数量，由管理员确认后写入。 |
| FR-EXCH-009 | 相同包重复导入不得生成重复数据；相同批次号但内容不同必须报批次冲突，不得复用旧结果。 |
| FR-EXCH-010 | 外部来源更新应更新漏洞当前来源事实及内容哈希；内部匹配阶段再重算 OTS/CVE 候选关系，任何更新均不得覆盖产品评估和审核记录。 |
| FR-EXCH-011 | 人工维护内容发生唯一键冲突时应提示，不得静默覆盖。 |
| FR-EXCH-012 | 导入错误应包含文件名、行号、字段和原因，并支持导出错误清单。 |
| FR-EXCH-013 | 外部数据服务应输出版本化 ZIP 数据包；NVD `1.0` 根目录恰好包含 `manifest.csv` 和 `nvd_cves.csv`，manifest 声明批次、生产者、来源发布标识、来源时间窗口和文件摘要。 |
| FR-EXCH-014 | 管理平台应校验 ZIP 安全、文件集合、编码/换行、固定表头、必填字段、JSON 结构、批次号、来源窗口和文件摘要；不得要求包内存在内网 OTS 引用。 |
| FR-EXCH-015 | NVD `1.0` 使用来源时间窗口表示本包覆盖区间，不把它解释为逐 OTS 完整覆盖；若后续格式引入按 OTS 采集状态，失败不得伪装为成功覆盖。 |
| FR-EXCH-016 | 管理平台新增、停用产品版本或调整产品 OTS 关联后，下一次导出的采集范围应相应增加或移除 OTS；移出范围不得删除其历史漏洞、候选关联或评估数据。 |
| FR-EXCH-017 | 采集范围中的 `last_covered_time` 可为空并由外部工具自行决定回溯起点；NVD 来源窗口和“没有新 CVE”都不能解释为某个 OTS 没有漏洞。 |
...
| 编号 | 需求 |
| --- | --- |
| FR-MATCH-001 | 管理平台应依据漏洞的公开软件身份、精确版本或版本范围与内部 OTS 名称/版本执行候选匹配，保存匹配方式、范围证据和可选置信度。 |
| FR-MATCH-002 | 管理平台应保存候选关联的首次/最近来源批次、匹配依据和内容哈希；候选关联不设置人工分析员确认或提交环节。 |
| FR-MATCH-003 | 内部候选匹配生成后，系统应为每个相关有效产品 OTS 创建独立产品评估任务，并将当前责任人设置为该产品版本指定的产品负责人。 |
...
2. CVE 与 OTS 是多对多关系：一个 CVE 可关联多个 OTS，一个 OTS 可关联多个 CVE；平台内部计算的候选匹配及依据保存在独立关联记录中；
...
14. NVD `1.0` 回传包按来源时间窗口提供原始 CVE 事实，不要求在外网侧按内网 OTS 过滤，也不得携带内网 OTS ID；
15. NVD 来源窗口与逐 OTS 覆盖是不同概念；只有后续格式明确记录某个 OTS 采集成功时，才可推进该 OTS 覆盖截止时间；
...
6. NVD `1.0` 包不包含内网 OTS 引用；无 configuration、Rejected 或暂时无内部 OTS 匹配的 CVE 仍可作为来源事实导入；
7. 管理平台能够校验、预览并事务导入两文件 NVD 包，重复包不产生重复数据，相同批次号不同内容被明确拒绝；
8. 管理平台能够区分 NVD 来源窗口和逐 OTS 覆盖，不以来源窗口推断某个 OTS 无漏洞；
9. 平台能够保存并区分 CVE、CWE、受影响范围、参考链接、CVSS v3.1、KEV 和 EOL；
10. EOL 候选信息能够由审核人确认或不采信；
11. 漏洞事实导入后，平台能够按受影响软件/版本范围匹配内部 OTS，再为使用候选 OTS 的多个有效产品版本生成相互独立的评估任务，并在对应产品负责人工作台显示；
