...
| 本版调整 | NVD 包改为先导入原始 CVE 事实和受影响软件/版本范围，再由内网平台匹配 OTS 并生成评估待办；KEV/EOL 作为后续可选格式；外网采集为项目边界外的外部数据服务 |

## 1. 方案结论

V1 采用“外部数据服务 + 离线 ZIP/CSV 数据包 + 内网管理平台”的形式。

- 外部数据服务生成指定来源时间窗口的 NVD 原始事实包；管理平台先导入 CVE，再依据受影响软件/版本范围匹配内部 OTS；
- 管理平台仍可导出实际在用 OTS 的采集范围，供外部工具选择性缩小采集量，但该文件不是 NVD `1.0` 回传包的一部分；
...
- 从 NVD 取得指定来源时间窗口的 CVE 事实；
- 生成符合第 3 章版本化 ZIP/CSV 契约的数据包；
- 在每条 CVE 中提供来源标识、描述、评分、CWE、参考、原始 configuration 和可归一化的受影响软件/版本范围；
- 可以选择读取管理平台导出的采集范围以优化采集，但不得要求或回传内网 OTS ID。

外部数据服务不维护内部 OTS/CVE 候选关系或产品评估结论，不访问内网数据库，不直接向管理平台推送。本项目不实现其 NVD、CISA KEV、endoflife.date 联网访问、分页、限流、重试、采集游标或日志。
...
- 支持漏洞事实导入、内部 OTS/CVE 候选匹配、评估待办生成，以及评估、审核和修订；
...
本项目不处理外部数据服务如何获取数据源。NVD `1.0` 包按来源时间窗口提供原始 CVE 事实，不要求外部服务识别内网 OTS，也不因当前没有内部 OTS 候选而丢弃 CVE。平台在事实导入完成后执行内部匹配。

#### NVD

- 外部服务提供 CVE ID、来源标识、来源状态、发布时间、最后修改时间、描述、全部 CWE、参考链接及来源 CVSS；
- 同一行同时保存原始 NVD configuration 和归一化的受影响软件、精确版本或版本区间；无 configuration 和 Rejected 记录也允许进入漏洞事实库；
- 外部包不包含内网 `ots_id` 或预先计算的候选匹配。平台在 OTS-08 中使用受影响软件身份和版本范围匹配 `ots_component`，匹配结果只作为候选线索。

#### CISA KEV

- V1.0 离线包暂不接收 KEV；确认启用后必须升级包格式版本，不保留空占位文件；
- KEV 通过 CVE ID 丰富已导入漏洞事实，不要求外部服务先匹配 OTS；
...
- 外部服务按公开软件身份和版本线提供可用的生命周期候选信息，不携带内网 OTS ID；
- 导入结果只作为候选信息；
- 审核人确认后才更新 OTS 的 `is_eol` 当前结论；
- 有供应商或项目官方来源时，人工补充并优先采用官方来源；
- 无可靠信息时不更新当前结论，也不推断为“未 EOL”。

NVD `1.0` manifest 记录来源名称、来源发布标识和来源时间窗口。来源窗口只说明本包覆盖的 NVD 数据区间，不证明某个内部 OTS 没有漏洞。若未来启用按 OTS 采集的 KEV/EOL 覆盖状态，应在后续格式中另行定义。
...
| 文件 | 内容 |
| --- | --- |
| manifest.csv | 包格式版本、批次号、生成时间、生产者版本、来源名称/发布标识、来源时间窗口和 `nvd_cves.csv` 摘要 |
| nvd_cves.csv | 一行一个 CVE；稳定标量列加受影响软件、CVSS、CWE、参考和原始 configuration 的 JSON 数组列 |

所有 CSV 使用 UTF-8 无 BOM、固定表头、ISO 8601 时间和稳定主键；记录分隔符使用 CRLF，带引号字段内部兼容 LF 或 CRLF。格式 `1.0` 根目录恰好包含以上两个文件，只接收 NVD 数据，不包含采集范围、KEV/EOL 或内网 OTS 标识。`nvd_cves.csv` 每行对应一个 CVE，一对多信息保存在 JSON 数组列中。
...
1. 上传数据包；
2. 校验格式、字段、JSON 结构、摘要、批次号和来源窗口；
3. 预览新增、更新、重复、冲突和错误数量；
4. 管理员确认后导入并查看结果。

导入规则：

- 相同包 SHA-256 重复上传返回已有批次；相同 `batch_no` 但包内容不同返回明确批次冲突，不回放旧错误；
- 校验 `manifest.csv` 声明的来源窗口和 `nvd_cves.csv` 原始字节摘要；
- 按 `cve_id` 与第 8 张表当前事实比较，预览新增、更新、重复、冲突和错误；确认时在锁内重新分类；
- CVE 及其 NVD JSON 来源事实、归一化受影响软件/版本范围更新到漏洞当前事实，并重算内容哈希；
- 来源合法更新可改变漏洞当前展示值，但不得覆盖候选匹配以外的产品评估和审核信息；
- 已提交或已审核的产品评估永不被导入覆盖；
- 人工维护数据与导入数据冲突时进入冲突清单，由管理员处理；
- 校验错误按文件名、行号、字段、错误值和原因导出；
- 正式导入使用批次事务，批量 upsert 第 8 张表并提交一条 `batch_upsert` 审计摘要；失败整批回滚，批次保留可识别的失败状态；
- 将来源发布标识和时间窗口保存到第 7 张表 `manifest_json`，`covered_from/covered_to` 表示来源窗口，不等价于逐 OTS 无漏洞证明；
- OTS-07 到此结束，不生成候选匹配或产品任务。OTS-08 随后根据第 8 张表受影响范围和第 5 张表内部 OTS 生成第 9 张表候选关系；OTS-10 再通过第 6 张表生成第 10 张表评估任务。
...
1. 外部数据服务提供 CVE 来源事实、公开软件身份和受影响版本范围，不提供内网 OTS ID；
2. 管理员校验并确认导入数据包后，平台先保存第 8 张表漏洞事实；
3. 平台按软件身份和版本范围与内部 `ots_component` 比较，生成第 9 张表候选匹配及依据；
4. 平台按候选关联查找该 OTS 对应的全部有效产品版本；
5. 对每个“产品 OTS + CVE”创建独立待评估任务，责任人取该产品版本当前指定产品负责人；
6. 事务提交后，待办立即出现在对应产品负责人工作台，作为站内通知；
7. 来源事实或候选匹配发生实质变化时，已完成结论保留，并为相关产品创建待复评修订。
...
V1 以每条当前产品评估持久化的规范基线 JSON 和 SHA-256 作为统一判定依据。基线只包含 CVE 状态、CVSS v3.1 分数及规范向量、规范化受影响范围、候选存在性/匹配方式/规范证据和产品 OTS 状态；只有已启用数据契约明确提供 KEV 时才包含 KEV。NVD `1.0` 未提供可信 KEV/EOL 输入，缺失值不得解释为 `false`，CVSS v4.0 和 EOL 不进入本版本指纹。

来源确认、候选执行、关联停用/恢复在各自业务事务内锁定当前评估并比较基线。`completed` 实质变化时保留原结论和审核留痕，使用统一克隆原语创建连续的 `reassess` 当前子修订并清空提交/审核事件；`pending/returned/reassess/submitted` 只合并有界变化摘要和更新基线，保持状态与用户内容并递增乐观锁。相同指纹重放为零写入，审计不保存完整前后正文或证据。

上线顺序为：先应用 `013` 迁移，再执行 `python run.py initialize-assessment-bases --dry-run` 核对数量，随后用稳定批次执行完整初始化并确认 `remaining_count=0`，最后开放自动入口。初始化可中断重跑，只填充当前修订空基线；完成前自动入口会拒绝评估写入。
...
`vulnerability.cve_id` 对漏洞主记录设置唯一约束，但“漏洞唯一”不等于“只影响一个 OTS”。CVE ID 回答“这是哪个漏洞”；`vulnerability_ots_match` 回答“平台根据来源受影响范围判断该漏洞可能与哪个内部 OTS 有关、依据是什么”。一个 CVE 可能影响多个使用相同易受攻击代码或依赖的 OTS，一个 OTS 也通常会对应多个 CVE，因此二者是多对多关系。若把单一 `ots_id` 放入 `vulnerability`，同一 CVE 关联第二个 OTS 时只能复制漏洞行或覆盖原 OTS，都会破坏 CVE 唯一性。
...
| 数据交换与漏洞 | `import_batch`、`vulnerability` | 导入批次、来源发布标识和时间窗口；CVE 当前来源事实、受影响软件/范围、原始 configuration、CWE/引用/CVSS JSON、KEV、内容哈希和可选 AI 通用建议 |
| 匹配与评估 | `vulnerability_ots_match`、`product_assessment` | 平台内部计算的 OTS/CVE 候选匹配及依据；产品独立评估、环境评分、修订链和审核结果 |
...
- `import_batch.manifest_json` 对 NVD `1.0` 保存来源发布标识和来源时间窗口；`scope_coverage_json` 仅为以后按 OTS 采集的可选格式保留；
- OTS-06 的 `last_covered_time` 在尚无按 OTS 覆盖数据时允许为空，不以 NVD 来源窗口冒充逐 OTS 完整覆盖；
...
- CVE/OTS 多对多候选关联、匹配后生成待办、内容哈希、评估修订和审核信息保存；
- 乐观锁并发更新冲突；
- 数据库变更动作类型和字段差异记录。
...
- NVD 两文件契约、来源发布标识、来源时间窗口和文件摘要；
- 含精确版本、开闭区间、无 configuration 和 Rejected CVE 的事实导入；
- 无内部 OTS 匹配的 CVE 可以导入且不产生候选关系；
- 平台内部软件身份/版本范围匹配及候选依据；
- 数据包重复导入、失败回滚和错误清单；
- 来源更新不覆盖已提交评估；
- 数据库备份和恢复。
...
- 内部候选匹配及任务生成完成后，相关任务出现在各产品版本负责人工作台，且重复处理不重复生成；
...
- 固定 NVD 原始事实的两文件输入契约和样例包；
- 来源窗口/摘要校验、真实数据库预览、事务导入、批次记录和错误清单；
- 平台内部 OTS/CVE 候选匹配；
- KEV/EOL 确认启用后再以新格式版本补充。

### 阶段 3：评估闭环

- OTS/CVE 候选匹配查询和基于候选关系生成评估待办；
- 产品评估和环境评分；
- 审核、退回、修订和待复评；
- 工作台、查询和追溯。
