...
#### [~] OTS-20 `ots-20-assessment-export`（复杂度：M，依赖：OTS-14/19；实现与验证已完成，待同步归档）
...
**实施证据（待归档确认）**：新增评估导出预检与 CSV 流式下载 API、“数据交换－评估导出”页面、OpenAPI 生成类型及系统 Chrome 纵向场景。后端全量 pytest 220 项通过，OTS-20 新增模块专项覆盖率 98%；前端 Vitest 142 项通过，总语句覆盖率 96.8%、分支 81.44%、函数 82.96%，新增导出 API 100%、页面 98.11%；类型检查、生产构建通过。真实 MySQL 8 验证 11 张基础表、204 行导出、并发修订提交前后均仅输出一个当前修订，计数和 keyset 查询使用既有索引且顺序查询耗时 0.125 秒，因此未新增索引或迁移。系统 Chrome OTS-20 专项通过；全量 35 项初跑发现导航增长导致 2 项回归，修复侧栏滚动与期望后相关 4 项复测通过。
