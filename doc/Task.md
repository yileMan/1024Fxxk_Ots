...
#### [x] OTS-20 `ots-20-assessment-export`（复杂度：M，依赖：OTS-14/19；已于 2026-09-06 归档）
...
**需求追溯**：FR-EXPORT-001 由指定单个产品版本与其关联 OTS、全部状态当前修订、排除历史修订和稳定排序覆盖；FR-EXPORT-002 由固定十九列 UTF-8 BOM CSV 覆盖来源/环境评分与向量、适用性、分析依据、处置、状态及提交审核信息，并对缺失值和特殊字符保持稳定表达；FR-AUDIT-003 由预检、成功、失败、拒绝和取消均使用只读会话且不接入 `audit_log` 覆盖。普通用户按当前有效产品版本授权，管理员全局只读；大结果按 CVE/评估 ID keyset 分批流式生成，不新增表、迁移、任务、临时文件或导出记录。

**完成证据**：实现提交 `d82fa36`；新增评估导出预检与 CSV 流式下载 API、“数据交换－评估导出”页面、OpenAPI 生成类型及系统 Chrome 纵向场景。后端全量 pytest 220 项通过，OTS-20 新增模块专项覆盖率 98%；前端 Vitest 142 项通过，总语句覆盖率 96.8%、分支 81.44%、函数 82.96%，新增导出 API 100%、页面 98.11%；类型检查、生产构建通过。真实 MySQL 8 验证 11 张基础表、204 行导出、并发修订提交前后均仅输出一个当前修订，计数和 keyset 查询使用既有索引且顺序查询耗时 0.125 秒，因此未新增索引或迁移。系统 Chrome Playwright 全量 35 项通过。主规格已同步，OpenSpec change 于 2026-09-06 归档为 `2026-09-06-ots-20-assessment-export`。

#### [x] OTS-21 `ots-21-audit-and-system-operations`（复杂度：L，依赖：OTS-02/10/15）
...
**需求追溯**：FR-AUDIT-001 由统一审计构造器、10 张非审计业务表写路径矩阵、四种限定动作和业务/审计同事务覆盖；FR-AUDIT-002 由管理员只读列表/详情、对象/用户/动作/闭区间筛选、`created_at/id` keyset 稳定分页、操作者与对象标识及版本化脱敏差异覆盖；FR-AUDIT-003 由登录、纯查询、导出、备份状态读取、健康/运行探测和失败回滚不写 `audit_log` 覆盖。系统方案 11.3 由应用版本、数据库、磁盘、最近备份、最近导入和最近失败六分项状态覆盖；详细状态仅管理员可读，公开健康接口保持最小响应。OTS-21 只读受限 `backup-status-v1.json`，未配置时明确显示未知，不执行备份或恢复，生产者实现留给 OTS-22。

**完成证据**：新增统一审计构造器、管理员审计列表/详情 API、“系统管理－变更记录”页面、详细运行信息 API 和“系统管理－系统运行”页面；现有用户、授权、产品、OTS、导入、候选匹配和评估写路径已接入同事务审计。后端全量 pytest 229 项通过；覆盖率回归总覆盖率 93%，新增审计构造器 90%、审计与运维服务 82%；前端 Vitest 147 项通过、总语句覆盖率 96.83%，新增审计 API 100%、审计页面 94.87%、运行状态页面 100%，类型检查与生产构建通过；系统 Chrome Playwright 37 项通过。OpenAPI/TypeScript 连续生成哈希一致。真实 MySQL 8 验证仍为 11 张业务表，5,000 条跨十年审计数据稳定分页无重复，20 并发查询总耗时 1.423 秒；执行计划使用既有 `idx_audit_time`、`idx_audit_user_time`，对象/动作时间查询扫描量可接受，因此未新增表、索引或迁移。主规格已同步，OpenSpec change 于 2026-09-06 归档为 `2026-09-06-ots-21-audit-and-system-operations`。剩余风险是应用层显式审计不能覆盖 DBA 绕过应用的直连 SQL，需由组织数据库审计承担；备份状态真实性与原子写入待 OTS-22 的交付脚本完成。
