...
- [x] 2.1 将格式 `1.0` 文件注册表、成员上限、manifest 文件闭包和摘要校验收敛为三个文件
- [x] 2.2 将领域校验和分类收敛为 `nvd_cves.csv` 单行 CVE 模型，严格解析 JSON 列并输出字段级错误
- [x] 2.3 保持上传流、ZIP 安全、批次幂等、归档清理、错误下载、管理员权限及无领域副作用行为不变，并更新 API/Repository 集成测试
- [x] 2.4 使用 MySQL 8.x 验证三文件成功/失败批次 JSON、唯一键恢复、11 张基础表数量和无迁移脚本
