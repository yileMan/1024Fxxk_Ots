...
- [x] 1.3 先编写 `nvd_cves.csv` 固定表头、基础字段、五个 JSON 数组字段、一行一个 CVE、重复/冲突及字段限制测试
...
- [x] 5.1 先编写默认 1 MiB 字段上限、恰好 1 MiB 可接受且超过 1 MiB 拒绝的 RED 测试
- [x] 5.2 将校验器、环境设置和示例配置的默认字段上限统一调整为 1,048,576 字节，其他资源限制保持不变
- [x] 5.3 使用最近一日真实 NVD 包确认 71,123 字节 configuration 不再触发字段超限，并确认后续候选匹配规则仍正常执行
- [x] 5.4 同步外部契约文档，运行后端覆盖率和 OpenSpec 严格校验并记录证据
